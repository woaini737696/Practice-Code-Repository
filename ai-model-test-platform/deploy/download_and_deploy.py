...
echo "=== 从workspace下载前端构建产物 ==="
curl -s -o /tmp/ai-build.tar.gz http://115.190.22.21:9999/ai-build.tar.gz 2>&1
ls -lh /tmp/ai-build.tar.gz
...
echo ""
echo "5. Auth登录:"
curl -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "username=admin&password=admin123" http://localhost/ai-test/api/auth/login | grep -q "access_token" && echo "✅ 登录正常" || echo "❌ 登录异常（admin可能不存在）"
...
print("从workspace下载构建产物到服务器...")
invoke_id = run_cmd(cmd, 180)
result = get_result(invoke_id, wait=40)
if result:
