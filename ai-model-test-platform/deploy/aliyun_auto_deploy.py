...
"""
    print("\n[1/5] 安装Docker环境...")
    invoke_id = run_command(client, INSTANCE_ID, step1_cmd, timeout=300)
    result = get_command_result(client, invoke_id, INSTANCE_ID)
    print(f"状态: {result['status']}, 退出码: {result['exit_code']}")
...
mkdir -p /opt/ai-model-test-platform/backend/app/api
mkdir -p /opt/ai-model-test-platform/backend/app/core
mkdir -p /opt/ai-model-test-platform/backend/app/models
mkdir -p /opt/ai-model-test-platform/backend/app/schemas
mkdir -p /opt/ai-model-test-platform/backend/app/services
mkdir -p /opt/ai-model-test-platform/backend/app/tasks
mkdir -p /opt/ai-model-test-platform/backend/app/websocket
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/auth
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/tests
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/models
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/distillation
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/scoring
mkdir -p /opt/ai-model-test-platform/frontend/src/pages/settings
...
"""
    print("\n[2/5] 准备项目目录...")
    invoke_id = run_command(client, INSTANCE_ID, step2_cmd, timeout=60)
    result = get_command_result(client, invoke_id, INSTANCE_ID)
    ...
    for local_rel, remote_path in files:
        ...
        if not os.path.exists(local_path):
            ...
            continue
        result = upload_file(client, INSTANCE_ID, local_path, remote_path)
        status = "OK" if result['status'] == 'Success' else f"FAIL({result['status']})"
        print(f"  {local_rel} -> {status}")
...
"""
    print("\n[4/5] 构建并启动服务...")
    invoke_id = run_command(client, INSTANCE_ID, step4_cmd, timeout=600)
    result = get_command_result(client, invoke_id, INSTANCE_ID)
    print(f"状态: {result['status']}")
    print(f"输出:\n{result['output']}")
...
"""
    print("\n[5/5] 配置Nginx...")
    invoke_id = run_command(client, INSTANCE_ID, step5_cmd, timeout=60)
    result = get_command_result(client, invoke_id, INSTANCE_ID)
    print(f"状态: {result['status']}")
    print(f"输出:\n{result['output']}")
