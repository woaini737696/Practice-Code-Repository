...
echo ""
echo "=== 使用printf生成Nginx配置（避免heredoc变量替换问题） ==="
...
echo ""
echo "6. Auth注册:"
curl -s -X POST -H "Content-Type: application/json" -d '{"username":"finaltest5","password":"testpass123"}' http://localhost/ai-test/api/auth/register

echo ""
echo "7. Auth登录:"
LOGIN=$(curl -s -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "username=finaltest5&password=testpass123" http://localhost/ai-test/api/auth/login)
